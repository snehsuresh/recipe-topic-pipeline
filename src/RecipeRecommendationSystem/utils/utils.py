...
def save_model(model, model_name):
    ...
    try:
        if s3_enabled:
            ...
            print("sneh pillai")
            dump(model, f"data/models/{model_name}.joblib")
            print(f"Model {model_name} saved to local fs in 'data/models' directory")
...
def load_model(model_name):
    try:
        ...
        if s3_enabled:
            ...
            model = load(f"data/models/{model_name}.joblib")
...
def save_dataframe(df, filename):
    ...
    try:
        if s3_enabled:
            ...
            print(f"DataFrame saved to local fs in 'data/processed' dir")
    except (NoCredentialsError, PartialCredentialsError) as e:
        print(f"Credentials error: {e}")
    except Exception as e:
        print(f"An error occurred: {e}")
